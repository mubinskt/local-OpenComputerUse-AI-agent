...
def build_prompt(instruction: str, width: int, height: int) -> str:
    return (
        "You are a local automation assistant. "
        "The user will provide an instruction and a screenshot description. "
...
def run_generation(instruction: str, output_text: tk.Text, screenshot_label: tk.Label) -> None:
    try:
        ...
        prompt = build_prompt(instruction, metadata["width"], metadata["height"])
...
def on_generate_click() -> None:
    ...
    if not instruction:
        ...
        return
    threading.Thread(target=run_generation, args=(instruction, output_area, screenshot_preview), daemon=True).start()
...
instruction_label.pack(anchor="w", padx=12, pady=(12, 0))

instruction_text = tk.Text(root, height=4, width=98)
instruction_text.pack(padx=12)
...
output_area = scrolledtext.ScrolledText(root, width=98, height=24)
